...
class Decoder(nn.Module):
    ...
    def forward(self, embedded_inputs,
        ...
        attentions = torch.cat([torch.unsqueeze(attention, dim=0) for attention in outputs], dim=0)
        attentions = torch.squeeze(attentions, dim=1)
        outputs = torch.cat(outputs).permute(1, 0, 2)
        pointers = torch.cat(pointers, 1)

        return (outputs, pointers), hidden, attentions
...
class PointerNet(nn.Module):
    ...
    def forward(self, inputs, get_attention=False):
        """
        ...
        if self.bidir:
            ...
            decoder_hidden0 = (encoder_hidden[0][-1],
                               encoder_hidden[1][-1])
        (outputs, pointers), decoder_hidden, attentions = self.decoder(embedded_inputs,
                                                           decoder_input0,
                                                           decoder_hidden0,
                                                           encoder_outputs)
        if get_attention:
            return outputs, pointers, attentions
        else:
